...
public class CMouse {
    ...
    public static boolean accurateInteract(GameObject gameObject, String action) {
        ...
        try {
            do {
                centerPoint =
                        gameObject.getModel().getInteractionPoint().getInteractionPoint();

                Execution.delay(150, 250);

                centerPoint2 = gameObject.getModel().getBoundingModel().getInteractionPoint();
    ...
    public static boolean accurateInteract(Npc npc, String action) {
        ...
        try {
            do {
                centerPoint = npc.getModel().getInteractionPoint().getInteractionPoint();

                Execution.delay(150, 250);

                centerPoint2 = npc.getModel().getInteractionPoint().getInteractionPoint();
            } while (centerPoint.distance(centerPoint2) > 10D);
        } catch (NullPointerException e) {
            return false;
        }
